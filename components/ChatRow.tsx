...
function ChatRow( { id_history, judul }: Props) {
    ...
    return (
        <Card m={1.5} bg={"transparent"} borderColor={"transparent"}>
            <HStack _hover={ { bg: "gray.500", opacity: 0.7 } } color={currentHistory && Number(currentHistory) === id_history ? "teal.600" : "gray.200"}  transition="all 200ms ease-out" border={"1px"} rounded={"lg"} display={"flex"} w={{base:"80vw", md:"20vw"}} h={{base:"6vh", md:"7.5vh"}} px={"1"} bgColor={currentHistory && Number(currentHistory) === id_history ? "teal.600" : "transparent"}>
